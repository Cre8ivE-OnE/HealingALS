...
Description: Allows you to protect and reliably serve up all kinds of files using MemberPress and Amazon AWS.
Version: 1.3.1
...
class MpawsAppController {
  ...
  private function av_shortcode( $av, $atts, $content='', $tag='' ) {
    ...
    foreach( $av_atts as $att ) {
      ...
      global $wp_version;
      // if ( $wp_version < 4.1 || $wp_version >= 4.1.2 ) {
        ...
        if(in_array($type['ext'], $exts)) {
          $funcs[] = (object)array(
            'mime' => create_function('$mt', 'return array_merge($mt, array(\''.preg_quote($fake_ext,'!').'\' => \'' . $type['type'] . '\'));'),
            'ext' => create_function('$e', 'return array_merge($e, array(\''.strtolower($fake_ext).'\'));')
          );
        }
      // }
    }
