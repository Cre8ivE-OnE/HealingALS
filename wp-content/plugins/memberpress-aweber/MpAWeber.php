...
class MpAWeber {
  ...
  public function __construct() {
    ...
    add_action('mepr-signup',                   array($this,'process_signup_optin'));

    // membership specific lists
    add_action('mepr-txn-status-complete',      array($this,'process_status_changes'));
    add_action('mepr-txn-status-confirmed',     array($this,'process_status_changes'));
    add_action('mepr-txn-status-failed',        array($this,'process_status_changes'));
    add_action('mepr-txn-status-refunded',      array($this,'process_status_changes'));
    //add_action('mepr-subscr-store',             array($this,'process_status_changes')); //Not necessary
  ...
  public function maybe_subscribe(MeprUser $contact, $list_id) {
    if(!$this->is_authorized()) { return false; }
    ...
    if(!$this->is_subscribed($contact, $list_id)) {
      ...
      $this->maybe_resubscribe($contact, $list_id);
    }

    return false;
  }

  public function maybe_unsubscribe(MeprUser $contact, $list_id) {
    if( !$this->is_authorized() ) { return false; }

    if($this->is_subscribed($contact, $list_id)) {
      return $this->unsubscribe($contact, $list_id);
    }
  ...
  public function process_signup_optin($txn) {
    $options = get_option('mepr_adv_aweber_options', array());
    $mepr_options = MeprOptions::fetch();
    ...
    if($this->is_enabled_and_authorized() and !empty($options)) {
      if( !$options['optin'] or
        ...
        $this->add_subscriber( $usr, $options['list'] );
      }
    }
  }
  ...
  public function process_status_changes($obj, $sub_status = false) {
    ...
    if(!$enabled or !is_array($options) or !isset($options['list']) or empty($options['list'])) {
      return;
    }
    ...
    if(in_array($obj->product_id, $usr->active_product_subscriptions('ids', true))) {
      $this->maybe_subscribe($usr, $options['list']);
    }
    else {
      $this->maybe_unsubscribe($usr, $options['list']);
    }
  }
